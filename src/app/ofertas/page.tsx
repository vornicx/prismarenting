...
export default function Offers() {
  ...
  const reset = () => { setQuery(""); setFuel("Todos"); setBody("Todos"); setBudget("Todos"); setSort("featured"); };

  return (
    <main className="catalog-page">
      <section className="catalog-stage">
        <Header />
        <div className="shell catalog-hero"><span className="eyebrow eyebrow-light">Ofertas de renting</span><h1>Elige menos.<br/><span>Elige mejor.</span></h1><p>Busca por modelo, filtra por lo que realmente condiciona la operación y entra en cada coche para ver la propuesta completa.</p></div>
      </section>
      <section className="shell catalog-controls">
        <div className="catalog-search"><label htmlFor="vehicle-search">Buscar coche</label><input id="vehicle-search" value={query} onChange={(event) => setQuery(event.target.value)} placeholder="BMW X5, Fiat 500, Hyundai…" /></div>
      ...
      <section className="shell catalog-results">
        ...
        {filtered.length ? <div className="catalog-grid">{filtered.map((vehicle) => <VehicleCard vehicle={vehicle} key={vehicle.slug} />)}</div> : <div className="catalog-empty"><h2>No hay coincidencias.</h2><p>Prueba a ampliar presupuesto o quitar algún filtro.</p><button className="button button-dark" onClick={reset}>Ver todas las ofertas</button></div>}
      </section>
    </main>
  );
}
