...
export default function OriginalVehicleTemplate({ product, page }: { product: OriginalProduct; page?: OriginalPage }) {
  ...
  return (
    <main className="source-page source-product-page">
      ...
      {html ? (
        <section className="source-shell source-original-content source-product-original-content"><div className="source-content-flow" dangerouslySetInnerHTML={{ __html: html }} /></section>
      ) : product.description ? (
        <section className="source-shell source-product-description"><h2>Información del vehículo</h2><p>{product.description}</p></section>
      ) : null}

      <section className="source-product-cta"><div className="source-shell source-product-cta-grid"><div><span>PRISMA Renting</span><h2>¿Te interesa este coche?</h2></div><div><p>Consulta disponibilidad, operador, kilometraje, plazo y condiciones finales con un asesor.</p><a href="tel:+34699242581">699 24 25 81</a></div></div></section>
      <OriginalFooter />
    </main>
  );
}
