...
const headerPrimary = [
  ...
  ["Cómo funciona", "/como-funciona-el-renting-de-prisma-renting/"],
  ["Para empresas", "/renting-coches-empresas/"],
    ...
    <header className="prisma-header">
      <div className="prisma-shell prisma-header-inner">
        <Link href="/" className="prisma-logo" aria-label="PRISMA Renting, inicio">
          <Image src="https://prismarenting.com/wp-content/uploads/2021/12/nuevo-logo-prisma-renting.webp" alt="PRISMA Renting" width={190} height={42} priority />
        </Link>

        <nav className="prisma-primary-nav" aria-label="Navegación principal">
          {headerPrimary.map(([label, href]) => <Link href={href} key={href}>{label}</Link>)}
        </nav>

        <div className="prisma-header-actions">
          <a className="prisma-header-phone" href="tel:+34699242581"><span>Asesoramiento</span><strong>699 24 25 81</strong></a>
          <Link className="prisma-header-cta" href="/contacto/">Quiero mi oferta</Link>
        </div>
        ...
        <details className="prisma-mobile-menu">
          ...
          <div className="prisma-mobile-panel">
            <nav aria-label="Navegación móvil">
              {headerPrimary.map(([label, href]) => <Link href={href} key={href}>{label}</Link>)}
              {renting.slice(1).map(([label, href]) => <Link href={href} key={href}>{label}</Link>)}
    ...
    <footer className="prisma-footer">
      <section className="prisma-footer-cta">
        <div className="prisma-shell prisma-footer-cta-grid">
          <div><span>¿No encuentras la operación que buscas?</span><h2>PRISMA también trabaja el renting a medida.</h2></div>
          <div>
            ...
            <div className="prisma-footer-cta-actions"><Link href="/contacto/">Pedir asesoramiento</Link><a href="tel:+34699242581">699 24 25 81</a></div>
          </div>
        </div>
      </section>
      ...
      <div className="prisma-shell prisma-footer-main">
        <div className="prisma-footer-brand">
          <Image src="https://prismarenting.com/wp-content/uploads/2021/12/nuevo-logo-prisma-renting.webp" alt="PRISMA Renting" width={220} height={50} />
          <p>Grupo PRISMA · Especialistas en automoción.</p>
          <p>Paseo Imperial 8, 1A · 28005 Madrid<br />699 24 25 81 · hola@prismarenting.com</p>
        </div>
        <div className="prisma-footer-column"><span>Renting</span>{renting.map(([label, href]) => <Link href={href} key={href}>{label}</Link>)}</div>
        <div className="prisma-footer-column"><span>Modalidades</span>{modalities.map(([label, href]) => <Link href={href} key={href}>{label}</Link>)}</div>
        <div className="prisma-footer-column"><span>PRISMA</span>{company.map(([label, href]) => <Link href={href} key={href}>{label}</Link>)}</div>
      </div>

      <div className="prisma-shell prisma-footer-directory">
        <details><summary>Explorar todas las categorías, marcas y modalidades <span>{discovered.length} rutas</span></summary><div className="prisma-footer-link-grid">{discovered.map((page) => <Link href={page.path} key={page.path}>{page.h1?.[0] || page.title.replace(/\s*[|–-].*$/, "")}</Link>)}</div></details>
      </div>

      <div className="prisma-shell prisma-footer-bottom">
        <span>© 2026 Grupo PRISMA, Especialistas en Automoción, S.L.</span>
        <div><Link href="/aviso-legal/">Aviso legal</Link><Link href="/politica-de-privacidad/">Privacidad</Link><Link href="/politica-de-cookies/">Cookies</Link></div>
      </div>
    </footer>
  );
}
