...
export default function OriginalCatalogClient({ products }: { products: OriginalProduct[] }) {
  ...
  useEffect(() => {
    ...
    if (q) setSearch(q);
    if (max && ["250", "300", "400", "500", "700", "1000"].includes(max)) setMaxPrice(max);
  }, []);
  ...
  const filterGroups = useMemo(() => {
    ...
    return [...groups.entries()]
      ...
      .sort((a, b) => ["Marca", "Tipo de coche", "Combustible", "Cambio", "Perfil", "Kilometraje", "Plazo"].indexOf(a.name) - ["Marca", "Tipo de coche", "Combustible", "Cambio", "Perfil", "Kilometraje", "Plazo"].indexOf(b.name))
      .slice(0, 7);
  }, [products]);
  ...
  useEffect(() => setVisibleCount(24), [search, maxPrice, filters, sort]);
  ...
  return (
    <div className="prisma-catalog-client">
      <aside className="prisma-catalog-filters" aria-label="Filtros del catálogo">
        <div className="prisma-filter-head">
          <div><span>Encuentra tu coche</span><strong>Filtrar catálogo</strong></div>
          <button type="button" onClick={reset}>Limpiar</button>
        </div>

        <label className="prisma-filter-search">
          <span>Buscar</span>
          <input value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Marca, modelo, SUV, híbrido..." />
        </label>

        <div className="prisma-budget-filter">
          <span>Cuota máxima</span>
          <div>{[
            ["all", "Todas"], ["250", "250 €"], ["300", "300 €"], ["400", "400 €"], ["500", "500 €"], ["700", "700 €"], ["1000", "1.000 €"],
          ].map(([value, label]) => <button type="button" className={maxPrice === value ? "is-active" : ""} onClick={() => setMaxPrice(value)} key={value}>{label}</button>)}</div>
        </div>

        <div className="prisma-filter-selects">
          {filterGroups.map((group) => (
            <label key={group.name}>
              <span>{group.name}</span>
              <select value={filters[group.name] || "all"} onChange={(event) => setFilters((current) => ({ ...current, [group.name]: event.target.value }))}>
      ...
      <div className="prisma-catalog-results">
        <div className="prisma-catalog-toolbar">
          <div>
            <span>Resultados</span>
            <strong>{result.length} vehículos</strong>
          </div>
          <label><span>Ordenar por</span><select value={sort} onChange={(event) => setSort(event.target.value as SortKey)}><option value="price-asc">Cuota: menor a mayor</option><option value="price-desc">Cuota: mayor a menor</option><option value="name">Marca y modelo</option></select></label>
        </div>
        ...
        {result.length ? (
          <>
            <div className="prisma-catalog-grid">{result.slice(0, visibleCount).map((product) => <OriginalVehicleCard product={product} key={product.path} />)}</div>
            {visibleCount < result.length && <div className="prisma-load-more"><button type="button" onClick={() => setVisibleCount((current) => current + 24)}>Mostrar 24 más <span>{result.length - visibleCount} restantes</span></button></div>}
          </>
        ) : (
          <div className="prisma-catalog-empty"><span>Sin coincidencias</span><strong>No hay vehículos que encajen con esos filtros.</strong><p>Prueba ampliando la cuota o eliminando algún criterio. También puedes pedir a PRISMA una búsqueda a medida.</p><button type="button" onClick={reset}>Ver catálogo completo</button></div>
        )}
      </div>
    </div>
  );
